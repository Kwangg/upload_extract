...
const toRel = (p: string) => path.relative(uploadsRoot, p).split(path.sep).join('/');
  ...
  await new Promise<void>((resolve, reject) => {
    yauzl.open(zipPath, { lazyEntries: true, decodeStrings: false }, (err, zipfile) => {
      ...
      zipfile.on('entry', (entry: any) => {
        ...
        if (!isDir && isEncrypted) encrypted.push(decoded);
        zipfile.readEntry();
      });
...
const sanitizeSegment = (s: string) => s.replace(/[<>:\"\/\\|?*\x00-\x1F]/g, '_').trim() || '_';
const sanitizeEntryPath = (p: string) => p.split('/').map(sanitizeSegment).join(path.sep);
const uniquePathIfExists = async (absTarget: string) => {
...
function getUserRoot(req: express.Request): string {
...
const resolveSafeForUser = (req: express.Request, rel: string) => {
...
const toRelForUser = (req: express.Request, p: string) => {
  const base = getUserRoot(req);
...
app.post('/extract', async (req, res) => {
  try {
    ...
    if (ext === '.rar') {
      ...
      const group = { zipRelative: fileName, extractRelative: toRelForUser(req, rarTargetDir), entries: [] as string[] };
      try {
        ...
        const allEntries = [...result.files].map((f: any) => f.fileHeader?.name).filter((n: any) => typeof n === 'string');
        if (targetEntry) {
...
app.post('/g23/preview', async (req, res) => {
  try {
    ...
    if (!fileName) return res.status(400).json({ ok: false, error: 'ไม่พบพาธไฟล์' });
...
app.get('/files/preview', async (req, res) => {
  try {
    ...
    const maxLines = Number((req.query.lines as string) || 200);
    ...
    if (stat.isDirectory()) {
      ...
      const children = await Promise.all(items.map(async (d) => {
        ...
        return { name: d.name, type: d.isDirectory() ? 'dir' : 'file', mtime: st.mtime.toISOString(), size: d.isDirectory() ? undefined : st.size } as { name: string; type: 'dir' | 'file'; mtime?: string; size?: number };
      }));
...
app.post('/files/zip', async (req, res) => {
  try {
    ...
    const zipName = body.zipName as string | undefined;
    ...
    if (sourceDir) {
      ...
      if (!stat || !stat.isDirectory()) return res.status(400).json({ ok: false, error: 'ต้องระบุโฟลเดอร์ที่มีอยู่จริง' });
      ...
      const walk = async (current: string, root: string) => {
        ...
        for (const e of entries) {
          ...
          if (e.isDirectory()) await walk(abs, root);
          else if (e.isFile()) {
    ...
    if (filesToZip.length === 0) return res.status(400).json({ ok: false, error: 'ไม่พบไฟล์ในรายการที่จะ zip' });
...
// Files: delete
const rimraf = async (p: string) => {
  try {
    const stat = await fsp.stat(p);
    if (stat.isDirectory()) {
      const entries = await fsp.readdir(p);
      for (const e of entries) await rimraf(path.join(p, e));
      await fsp.rmdir(p);
    } else {
      await fsp.unlink(p);
    }
...
app.get('/sql/summary', async (req, res) => {
  try {
    ...
    for await (const line of rl as any) {
      const m = re.exec(line as string);
      if (m && m[1]) {
        const table = m[1];
