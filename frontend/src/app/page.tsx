...
// Safe dirname for relative paths using forward slashes
...
export default function UploadPage() {
    ...
    for (let i = 0; i < allEntries.length; i++) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setExtractedFiles(prev => [...prev, ...normalized]);
          setExtractedGroups(prev => [...prev, ...(json.extractedGroups||[])]);
          // แตกไฟล์ซ้อนทันทีสำหรับไฟล์ archive ที่เพิ่งแตกออกมา
          const nestedCandidates = normalized.filter((p: string) => [".zip", ".rar"].some((ext) => p.toLowerCase().endsWith(ext)));
  ...
  const extractNestedArchive = async (archiveRelative: string) => {
    ...
    const outputDir = dirOf(archiveRelative);
    ...
    try {
      ...
      if (res.ok) {
        ...
        const baseRel = Array.isArray(json.extractedGroups) && json.extractedGroups[0]?.extractRelative
          ? json.extractedGroups[0].extractRelative
          : isZip ? dirOf(archiveRelative) : (outputDir || dirOf(archiveRelative));
        const normalized = (json.extractedFiles || []).map((e:string) => `${baseRel}/${e}`.replace(/\\/g, "/"));
        setExtractedFiles((prev) => [...prev, ...normalized]);
        setExtractedGroups((prev) => [...prev, ...(json.extractedGroups || [])]);
        // แตกไฟล์ซ้อนเพิ่มเติมทันทีสำหรับไฟล์ที่เป็น archive (.zip/.rar) ที่เพิ่งแตกออกมา
        const nestedCandidates = normalized.filter((p: string) => [".zip", ".rar"].some((ext) => p.toLowerCase().endsWith(ext)));
        for (const rel of nestedCandidates) {
          await extractNestedArchive(rel);
        }
  ...
  const extractWithPassword = async (fileName: string, pwd: string, outputDir?: string) => {
    try {
      ...
      if (res.ok) {
        ...
        const baseRel = Array.isArray(json.extractedGroups) && json.extractedGroups[0]?.extractRelative
          ? json.extractedGroups[0].extractRelative
          : outputDir || dirOf(fileName);
        const normalized = (json.extractedFiles || []).map((e:string) => `${baseRel}/${e}`.replace(/\\/g, "/"));
        setExtractedFiles((prev) => [...prev, ...normalized]);
        setExtractedGroups((prev) => [...prev, ...(json.extractedGroups || [])]);
        // แตกไฟล์ซ้อนอัตโนมัติสำหรับไฟล์ที่เป็น .zip/.rar ที่ได้จากการแตกด้วยรหัสผ่าน
        const nestedCandidates = normalized.filter((p: string) => [".zip", ".rar"].some((ext) => p.toLowerCase().endsWith(ext)));
        for (const rel of nestedCandidates) {
          await extractNestedArchive(rel);
        }
  ...
  const onUpload = async () => {
            ...
            try {
              ...
              if (xhr.status >= 200 && xhr.status < 300) {
                if (data.requiresPassword) {
                  ...
                  if (lowerRel.endsWith('.zip') || lowerRel.endsWith('.rar')) {
    ...
    const groupsForAuto = extractedGroups.length > 0 ? extractedGroups : aggregatedGroups;
    if (groupsForAuto.length > 0) {
      await autoExtractAll(groupsForAuto);
    }
    // แตกไฟล์ .zip/.rar ที่อยู่ในรายการไฟล์ที่อัพโหลดแล้วอีกครั้ง (ตามคำขอ)
    if (zipsAfterUpload.length > 0) {
      for (const rel of zipsAfterUpload) {
        await extractNestedArchive(rel);
      }
    }
    // ตรวจหาไฟล์ .zip/.rar ที่อยู่ภายในผลการแตก (entries) แล้วแตกไฟล์ซ้อนเพิ่มเติม
    const groupsForNested = extractedGroups.length > 0 ? extractedGroups : aggregatedGroups;
    if (groupsForNested.length > 0) {
      const nestedArchiveRelatives = groupsForNested.flatMap((g) =>
        g.entries
          .filter((e) => {
      ...
      for (const nz of nestedArchiveRelatives) {
  ...
  return (
    <main style={{ maxWidth: 1080, margin: "24px auto", padding: 16 }}>
      ...
      {(() => {
        ...
        if (!showRight) {
          // ยังไม่มีไฟล์อัพโหลด/รายละเอียด: แสดงการ์ดเดียวกลางหน้า (จัดให้อยู่กึ่งกลางหน้าจอ)
          return (
